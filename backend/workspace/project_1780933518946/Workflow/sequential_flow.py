...
class SequentialStepParser:
    ...
    解析规则：
    1. 在文本中查找 `result:`、`next_agent:`、`next_task:`、`should_stop:` 前缀行
    2. 字段不区分大小写，值取冒号后的内容
    3. 若未找到对应字段则使用默认值
    """

    def parse(self, agent_type: str, raw_text: str) -> ParsedFlowStep:
        result = self._extract_field(raw_text, "result", raw_text or "none")
        ...
        return ParsedFlowStep(
            state=AgentState(output=AgentOutput(result=result)),
...
class SequentialFlow(FlowMemoryMixin, BaseFlow):
    ...
    def execute(
        ...
        **kwargs: Any,
        ...
        accumulated_context = request_text
        ...
        for idx, agent_key in enumerate(self.config.agent_order, start=1):
            ...
            # Build task context: original request + accumulated upstream results
            if idx == 1:
                task_text = request_text
            else:
                task_text = (
                    f"原始用户请求：{request_text}\n\n"
                    f"上游已完成的阶段与分析结果：\n{accumulated_context}\n\n"
                    f"请根据你的职责继续分析。"
                )
            ...
            # Accumulate context: append this agent's result to accumulated_context
            result_text = turn.parsed.state.output.result
            if not result_text or result_text == "none":
                result_text = turn.raw_output
