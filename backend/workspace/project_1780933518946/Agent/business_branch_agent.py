...
@dataclass
class BusinessBranchAgentConfig:
    """BusinessBranchAgent 的运行参数。"""

    prompt_file: str = "business_branch_agent.md"
...
class BusinessBranchAgent(BaseAgent):
    """
    负责"business_branch"的 agent。
    ...
    def __init__(
        ...
        config: Optional[BusinessBranchAgentConfig] = None,
        prompt_loader: Optional[PromptLoader] = None,
    ) -> None:
        super().__init__(
            agent_type="business_branch",
            model=model,
            config=config or BusinessBranchAgentConfig(),
    ...
    def run(self, user_input: str, **kwargs: Any) -> str:
        ...
        if runtime_system_prompt:
            system_prompt = system_prompt + chr(10) + chr(10) + runtime_system_prompt
