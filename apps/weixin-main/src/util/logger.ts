...
/**
 * Standalone connector logger — writes JSON lines to:
 *   <tmpDir>/weixin-ilink-YYYY-MM-DD.log
 */

const MAIN_LOG_DIR = process.env.WEIXIN_LOG_DIR?.trim() || path.join(os.tmpdir(), "weixin-ilink");
const SUBSYSTEM = "weixin-ilink";
const RUNTIME = "node";
const RUNTIME_VERSION = process.versions.node;
const HOSTNAME = os.hostname() || "unknown";
const PARENT_NAMES = ["weixin"];
...
function resolveMainLogPath(): string {
  ...
  return path.join(MAIN_LOG_DIR, `weixin-ilink-${dateKey}.log`);
}
