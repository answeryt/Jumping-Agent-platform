...
describe("sanitizeBotAgent", () => {
  it("returns default when input is empty / undefined / whitespace", () => {
    expect(sanitizeBotAgent(undefined)).toBe("WeixinConnector");
    expect(sanitizeBotAgent("")).toBe("WeixinConnector");
    expect(sanitizeBotAgent("   ")).toBe("WeixinConnector");
    expect(sanitizeBotAgent("\t\n")).toBe("WeixinConnector");
  });
  ...
  it("falls back to default when all tokens are invalid", () => {
    expect(sanitizeBotAgent("garbage no slashes here")).toBe("WeixinConnector");
  });

  it("drops non-ASCII characters by failing the token regex", () => {
    // "中文/1.0" contains non-ASCII chars in name → token rejected.
    expect(sanitizeBotAgent("中文/1.0")).toBe("WeixinConnector");
  ...
  it("rejects tokens with too-long name or version", () => {
    ...
    expect(sanitizeBotAgent(`${longName}/1.0`)).toBe("WeixinConnector");
    const longVersion = "1".repeat(33);
    expect(sanitizeBotAgent(`MyBot/${longVersion}`)).toBe("WeixinConnector");
  });
...
describe("readPackageJsonFromDir", () => {
  ...
  beforeEach(() => {
    tmpRoot = fs.mkdtempSync(path.join(os.tmpdir(), "weixin-ilink-pkg-test-"));
  });
  ...
  it("finds package.json from compiled layout (dist/src/api/)", () => {
    ...
    writePkg(tmpRoot, {
      name: "@tencent-weixin/weixin-ilink",
      version: "2.4.2",
      ilink_appid: "bot",
    });
    ...
    expect(result.name).toBe("@tencent-weixin/weixin-ilink");
  });

  it("finds package.json from dev layout (src/api/)", () => {
    writePkg(tmpRoot, {
      name: "@tencent/weixin-ilink",
      version: "2.4.2",
      ilink_appid: "bot",
    });
