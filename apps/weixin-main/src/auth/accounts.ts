...
function resolveWeixinStateDir(): string {
  return path.join(resolveStateDir(), "weixin-ilink");
}
...
export type WeixinSectionConfig = WeixinAccountConfig & {
  accounts?: Record<string, WeixinAccountConfig>;
};

export type WeixinRuntimeConfig = {
  accounts?: Record<string, WeixinAccountConfig>;
  channels?: Record<string, WeixinSectionConfig | undefined>;
  botAgent?: string;
  routeTag?: number | string;
} & WeixinSectionConfig;

function resolveConfigSection(cfg?: WeixinRuntimeConfig): WeixinSectionConfig | undefined {
  return cfg?.channels?.weixin ?? cfg;
}

export function loadConfigRouteTag(accountId?: string, cfg?: WeixinRuntimeConfig): string | undefined {
  const section = resolveConfigSection(cfg);
  const accountTag = accountId ? section?.accounts?.[accountId]?.routeTag : undefined;
  const value = accountTag ?? section?.routeTag ?? process.env.WEIXIN_ROUTE_TAG;
  if (typeof value === "number") return String(value);
  return typeof value === "string" && value.trim() ? value.trim() : undefined;
}

export function loadConfigBotAgent(cfg?: WeixinRuntimeConfig): string | undefined {
  const section = resolveConfigSection(cfg);
  const value = section?.botAgent ?? process.env.WEIXIN_BOT_AGENT;
  return typeof value === "string" && value.trim() ? value.trim() : undefined;
}

/** List accountIds from the index file (written at QR login). */
export function listWeixinAccountIds(_cfg?: WeixinRuntimeConfig): string[] {
...
export function resolveWeixinAccount(
  cfg: WeixinRuntimeConfig | undefined,
  ...
  return {
    accountId: id,
    baseUrl: stateBaseUrl || accountCfg.baseUrl?.trim() || DEFAULT_BASE_URL,
