...
用法：
    ...
    python agent_builder/config_creator/create_config.py my_agent --model gpt-4o

会自动生成（写入沙盒容器 /workspace/<project_name>/Config/）：
    Config/model_config.toml
    Config/tool_config.toml
"""
...
from config_template.config_templete import model_config_toml, tool_config_toml
...
def create_config(project_name: str, model_name: str = "deepseek-chat", executor: SandboxExecutor | None = None) -> None:
    ...
    config_dir = f"/workspace/{name}/Config"

    model_path = f"{config_dir}/model_config.toml"
    tool_path = f"{config_dir}/tool_config.toml"
    ...
    if check.returncode == 0:
        ...
        result = exec_.write_file(model_path, model_config_toml(model_name))
        if result.ok:
    ...
    # 写入 tool_config.toml
    check = exec_.run(["test", "-f", tool_path])
    if check.returncode == 0:
        print(f"已存在，跳过：{tool_path}")
    else:
        result = exec_.write_file(tool_path, tool_config_toml())
        if result.ok:
            print(f"已生成：{tool_path}")
        else:
            print(f"写入失败：{tool_path}\n{result.stderr}", file=sys.stderr)
...
def main() -> None:
    ...
    create_config(args.project_name, args.model)
