...
会自动生成（写入本地工作区 /workspace/<project_name>/runtime/Config/）：
...
def create_config(project_name: str, executor: ExecutorProtocol | None = None) -> None:
...
def main() -> None:
    parser = argparse.ArgumentParser(description="生成 Config 配置文件骨架")
